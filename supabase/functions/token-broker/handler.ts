 ...
 *
 * Paths:
 *   - OAuth unconfigured (local/dev): POST → hand-minted HS256 `{ token }`.
 *   - OAuth configured + stub-token POST: rejected (403) — no production HS256 mint.
 *   - OAuth configured + real TeamUp JWT or OAuth callback: Auth session ES256
 *     `{ access_token, refresh_token, expires_at, token }`. Requires Custom Access
 *     Token Hook so app_metadata is promoted to top-level claims.
 *
 ...
 * vars, only the POST stub path is available.
 *
 ...
 *   JWT_SIGNING_SECRET  — HS256 session mint only when OAuth is unconfigured (local/dev).
 *                         Not used on the real OAuth / Auth-session path.
 *   OAUTH_STATE_SECRET  — HMAC for OAuth `state` (authorize/callback). Required when
 *                         TeamUp OAuth is configured. Must NOT be the project JWT secret.
 *
...
async function issueMemberSession(
  ...
  // When OAuth is configured, callers must reject stub-token before this path;
  // useAuthSession is then always true for remaining POSTs.
  if (!useAuthSession) {
...
async function handlePost(req: Request): Promise<Response> {
  ...
  // Production (OAuth configured): refuse stub-token — no HS256 session mint.
  // Local/dev (OAuth unconfigured): stub-token still takes the HS256 path below.
  if (isStubTokenRejectedWhenOAuthConfigured(request.teamupToken)) {
    return jsonResponse(
      {
        error:
  ...
  // Auth session when OAuth is configured (stub path already rejected above).
...
async function handleOAuthCallback(req: Request): Promise<Response> {
  ...
  let state;
  try {
    state = await verifyOAuthState(stateToken, stateSecret);
  } catch {
...
/**
 * Top-level HTTP router for the token broker.
 * Exported so Deno tests can POST through the real path without Deno.serve.
 */
export async function handleBrokerRequest(req: Request): Promise<Response> {
...
/** Exported for HTTP-level unit tests that assert status/body without Deno.serve. */
export { handlePost };
