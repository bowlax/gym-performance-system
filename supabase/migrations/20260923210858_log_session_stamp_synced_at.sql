-- Stamp synced_at on a new web session (#23 follow-up)
--
-- log_session_atomic inserted the session, exercise entries, and sets with
-- synced_at left null. iOS pull only reads rows whose synced_at is later
-- than the last-pull marker, so those sessions never reached the phone.
-- add_exercises_to_session_atomic already stamps this column. This replaces
-- the function so a new session does the same, with one write_at for
-- updated_at and synced_at on the session, each entry, and each set.
...
begin
  ...
  for exercise_index in 0 .. exercise_count - 1 loop
    ...
    begin
      ...
      values (entry_id, jwt_gym_id, session_id, exercise_id, write_at, write_at)
      on conflict (id) do nothing;
    exception
      when unique_violation then
        raise exception 'Conflict' using errcode = 'PT409';
