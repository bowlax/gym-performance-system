...
- **log-set and related Edge Functions** - server-side write paths for web members (`log-session` for atomic multi-exercise creates, `add-exercises-to-session` for attaching to a live session, plus `log-set`, `add-manual-pb`, `update-manual-pb`, `reset-current-pb`, `delete-personal-best`, `delete-session`)
